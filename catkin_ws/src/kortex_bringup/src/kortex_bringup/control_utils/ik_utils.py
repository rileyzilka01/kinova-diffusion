...
class png_control(KinovaGen3):
    def __init__(self, args):
        ...
        self.ee_flat = np.zeros(7) # placeholder vector to operate last joint to keep it flat
        self.lengths = np.array([156.4, 128.4, 210.4, 210.4, 208.4, 105.9, 105.9, 61.5, 140]) # lengths of robot joints, could replace with dh-params
        self.last_j6_error = np.pi # for pid controller to keep ee flat
        self.j6_angle = np.pi # to rotate inputs into more intuitive reference frame
        self.rotation_gain = 0.2 # gains for rotations
        self.translation_gain = 0.2 # gains for translations
        self.wrist_motion_gain = 0.2 # gains for wrist flexion
    ...
    def step(self, ax, mode, custom_commands):
        ...
        if self.run:
            ...
            for custom_command in custom_commands:
                ...
                cmds_custom.append(self.remap_axes(custom_command.ax, custom_command.mode))
...
# Inference modified cartesian control
class cartesian_control(KinovaGen3):
    def __init__(self, args):
        # global 
        super(cartesian_control, self).__init__()
    ...
    def step(self, ax, mode, custom_commands):
        ...
        if self.run:
            ...
            self.send_cartesian_velocity(cmd_total)
    ...
    def remap_axes(self, vec, mode):
        if self.DOF == 3:
            if mode == 0:	
                cmd = [vec[1], vec[0], 0.6*vec[5], 0, 0, 0]
            elif mode == 1:
                cmd = [0, 0, 0, vec[1], -1.0 * vec[0], -1.0 * vec[2]]
...
# xbox controller so we can do cartesian and orientation at the same time
class xbox_control(KinovaGen3):
    ...
    def step(self, ax, custom_commands):
        if self.run:
